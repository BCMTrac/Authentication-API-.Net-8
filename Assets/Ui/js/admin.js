document.addEventListener('DOMContentLoaded', () => {
  ...
  async function apiFetch(url, options = {}) {
    ...
    if (res.status === 401) {
      try {
        const rr = await fetch(`${API.auth}/refresh`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(auth.refresh ? { refreshToken: auth.refresh } : {}) });
        if (!rr.ok) throw new Error('refresh failed');
        const json = await rr.json();
        if (json.token) saveTokens(json.token, json.refreshToken || auth.refresh);
