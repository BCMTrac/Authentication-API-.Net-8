function initMfaManagement(mfaManagementView, isEnabled) {
    ...
    async function startMfaEnrollment(e) {
        handleApiAction(async () => {
            ...
            enroll.innerHTML = `<p>1. Scan this QR code with your authenticator app:</p><div class="text-center"><img src="/api/v1/mfa/qr?otpauthUrl=${encodeURIComponent(data.otpauthUrl)}" alt="MFA QR Code"></div><p class="mt-3">2. Enter the code from your app to confirm:</p><form id="confirm-mfa-form"><div class="input-group mb-3"><input type="text" id="mfa-confirm-code" class="form-control" placeholder="6-digit code" required autocomplete="one-time-code"><button type="submit" class="btn btn-primary">Confirm & Enable</button></div></form>`;
    ...
    async function confirmMfaEnrollment(e) {
        ...
        await handleApiAction(async () => {
            ...
            document.getElementById('mfa-done-btn').addEventListener('click', () => initMfaManagement(mfaManagementView, true));
            return resp;
        }, null, button);
    }
